...
class UpdateBookingFull(BaseEndpoint):
    @allure.step('Send update request')
    def update_booking_full(self, booking_id, token, body, base_url=BaseEndpoint.BASE_URL):
        headers = {'Cookie': f'token={token}'}
        body = body if body else BaseEndpoint.BODY
        self.response = requests.put(f"{base_url}/booking/{booking_id}", json=body, headers=headers)
